...
def main(args):
    ...
    if args.reparam_syn:
        ...
        for f in args.files_name.split(','):
            if ema_images_best:
                ema_img = torch.cat([ema_images_best[-1], torch.load(os.path.join(image_path, f, 'ema_images_best.pt')).to(args.device).requires_grad_(False)], dim=0)
                ema_lab = torch.cat([ema_labels_best[-1], torch.load(os.path.join(image_path, f, 'ema_labels_best.pt')).to(args.device)], dim=0)
            else:
                ema_img = torch.load(os.path.join(image_path, f, 'ema_images_best.pt')).to(args.device).requires_grad_(False)
                ema_lab = torch.load(os.path.join(image_path, f, 'ema_labels_best.pt')).to(args.device)
            ema_images_best.append(ema_img)
            ema_labels_best.append(ema_lab)
        print('load ema_images_best ...')
    ...
    for k, inter in enumerate(intervals):
        ...
        ema = ModelEmaV2([image_syn], decay=args.ema_decay) # ema modi
        ...
        for it in range(0, args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    ema_accs_train = []
                    for it_eval in range(args.num_eval):
                        ema_net_eval = get_network(model_eval, channel, num_classes, im_size).to(args.device) # get a random model
                        if args.reparam_syn:
                            for img, lab, lr in zip(ema_images_best, ema_labels_best, args.lrs_net):
                                ema_eval_labs = lab
                                args.lr_net = lr
                                with torch.no_grad():
                                    ema_image_syn = ema.module[0]
                                    ema_image_save = ema_image_syn
                                ema_image_syn_eval, ema_label_syn_eval = copy.deepcopy(ema_image_save), copy.deepcopy(
                                    ema_eval_labs.detach())  # avoid any unaware modification
                                ema_net_eval, ema_acc_train, ema_acc_test = evaluate_synset(it, ema_net_eval, ema_image_syn_eval,
                                                                                 ema_label_syn_eval, testloader, args,
                                                                                 texture=args.texture, printer=True)
                            ema_eval_labs = torch.cat([ema_labels_best[-1], label_syn], 0)
                            with torch.no_grad():
                                ema_image_save = torch.cat([ema_images_best[-1], ema_image_syn], 0)
                        else:
                        ...
                        ema_image_syn_eval = copy.deepcopy(ema_image_save.detach())
                        ema_label_syn_eval = copy.deepcopy(ema_eval_labs.detach())
                        args.lr_net = syn_lr.item()
                        _, ema_acc_train, ema_acc_test = evaluate_synset(it_eval, ema_net_eval, ema_image_syn_eval, ema_label_syn_eval, testloader, args, texture=args.texture, printer=True)
                    ...
                    if ema_acc_test_mean > ema_best_acc[model_eval]:
                        ...
                        save_this_it = True
                    print('Original Evaluate %d random %s, mean = %.4f std = %.4f\n-------------------------'%(len(accs_test), model_eval, acc_test_mean, acc_test_std))
                    print('EMA Evaluate %d random %s, mean = %.4f std = %.4f\n-------------------------' % (
                    len(ema_accs_test), model_eval, ema_acc_test_mean, ema_acc_test_std))
                    wandb.log({'EMA_Max_Accuracy/{}'.format(model_eval): ema_best_acc[model_eval]}, commit=commit)  # modi
                    wandb.log({'EMA_Max_Std/{}'.format(model_eval): ema_best_std[model_eval]}, commit=commit)  # modi
            ...
            if it in eval_it_pool and (save_this_it or it % 1000 == 0):
                with torch.no_grad():
                    ...
                    ema_save_dir = os.path.join(args.save_dir, args.dataset, args.run_name, seg_path)
                    ...
                    if args.ipc < 50 or args.force_save:
                        ...
                        wandb.log({seg_path+"/ema_Synthetic_Images": wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                        wandb.log(
                            {seg_path+'/ema_Synthetic_Pixels': wandb.Histogram(torch.nan_to_num(ema_image_save.detach().cpu()))},
                            step=it)
                        ...
                        for clip_val in [2.5]:
                            ...
                            if args.dataset != "ImageNet":
                                ...
                                upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                            grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                            wandb.log({seg_path+"/ema_Clipped_Synthetic_Images/std_{}".format(clip_val): wandb.Image(
                        ...
                        if args.zca:
                            ...
                            if args.dataset != "ImageNet":
                                ...
                                upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                            grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                            wandb.log({seg_path+"/ema_Reconstructed_Images": wandb.Image(torch.nan_to_num(grid.detach().cpu()))},
                                      step=it)
                            wandb.log({seg_path+'/ema_Reconstructed_Pixels': wandb.Histogram(
                                torch.nan_to_num(ema_image_save.detach().cpu()))}, step=it)

                            for clip_val in [2.5]:
                                std = torch.std(ema_image_save)
                                mean = torch.mean(ema_image_save)
                                upsampled = torch.clip(ema_image_save, min=mean - clip_val * std,
                                                       max=mean + clip_val * std)
                                if args.dataset != "ImageNet":
                                    upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=2)
                                    upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                                grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                                wandb.log({seg_path+"/ema_Clipped_Reconstructed_Images/std_{}".format(clip_val): wandb.Image(
                                    torch.nan_to_num(grid.detach().cpu()))}, step=it)

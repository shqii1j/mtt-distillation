...
from utils import get_dataset, get_network, get_eval_pool, evaluate_synset, get_time, DiffAugment, ParamDiffAug, TensorDataset, get_daparam, epoch
import wandb
import copy
import random
import math
...
def main(args):
    ...
    if args.intervals:
        ...
        intervals = [(i, i + args.interval - 1) if i + args.interval <= args.max_end_epoch else (i, args.max_end_epoch)
                     for i in range(0, args.max_end_epoch - args.overlap + 1, args.interval - args.overlap)]
        print(f'Interval: {args.interval}')
        print(f'Intervals: {intervals}')
    else:
        interval = math.ceil((args.max_end_epoch + 1 + args.overlap * (args.segments - 1)) / args.segments)
        print(f'Interval: {interval}')
        intervals = [(i, i + interval - 1) if i + interval <= args.max_end_epoch else (i, args.max_end_epoch) for i in
                     range(0, args.max_end_epoch - args.overlap + 1, interval - args.overlap)]
        print(f'Intervals: {intervals}')
    ...
    for k, inter in enumerate(intervals):
        ...
        if k == 0:
            ...
            converted_weights = {}
            k = 0
            for name, param in get_network(args.model, channel, num_classes, im_size).state_dict().items():
                l = len(param.view(-1))
                converted_weights[name] = student_params[-1][k:k+l].view_as(param)
                k = k+l
            ...
            args.train_dc_aug_param = get_daparam(args.dataset, args.model, args.model, None)
            args.train_dc_aug_param['strategy'] = 'crop_scale_rotate'  # for whole-dataset training
            print('DC augmentation parameters: \n', args.train_dc_aug_param)

            for iit in range(0, args.num_experts):

                ''' Train real data '''
                teacher_net = get_network(args.model, channel, num_classes, im_size).to(
                    args.device)  # get a random model
                teacher_net.train()
                teacher_net.load_state_dict(converted_weights)
                lr = args.lr_teacher
                ...
                for e in range(args.train_epochs):
                    ...
                    train_loss, train_acc = epoch("train", dataloader=trainloader, net=teacher_net,
                                                  ...
                                                  criterion=train_criterion, args=args, aug=True)

                    test_loss, test_acc = epoch("test", dataloader=testloader, net=teacher_net, optimizer=None,
                                                criterion=train_criterion, args=args, aug=False)

                    print("Itr: {}\tEpoch: {}\tTrain Acc: {}\tTest Acc: {}".format(iit, e, train_acc, test_acc))
                ...
                train_trajectories.append(timestamps)

                if len(train_trajectories) == args.save_interval:
                    n = 0
                    while os.path.exists(os.path.join(save_dir, "replay_buffer_{}.pt".format(n))):
                        n += 1
                    print("Saving {}".format(os.path.join(save_dir, "replay_buffer_{}.pt".format(n))))
                    torch.save(train_trajectories, os.path.join(save_dir, "replay_buffer_{}.pt".format(n)))
                    buffer = buffer + torch.load(os.path.join(save_dir, "replay_buffer_{}.pt".format(n)))
                    train_trajectories = []

            for _ in train_trajectories:
                del _
